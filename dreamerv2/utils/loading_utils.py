...
def load_ep(basedir, which_ep, buffer_name='train_episodes', timestamp=None):
...
def load_agent(basedir, checkpoint_name='variables_train_agent.pkl',
  ...
  with open(f'{for_ckpt_dir}/action_space_variables.pkl', 'rb') as f:
    action_space = pickle.load(f)

  # with open(f'{for_ckpt_dir}/obs_space_{checkpoint_name}', 'rb') as f:
  with open(f'{for_ckpt_dir}/obs_space_variables.pkl', 'rb') as f:
  ...
  env_num = 0
  os.makedirs(os.path.join(logdir, f'train_episodes'), exist_ok=True)
  traj_dir = f"{basedir}/train_episodes/"
  eps = os.listdir(traj_dir)
  eps.sort()
  for traj_name in eps[:2]:
    shutil.copy(os.path.join(traj_dir, traj_name), os.path.join(logdir, f'train_episodes', traj_name))

  replay_dir = pathlib.Path(f'{logdir}/train_episodes')
  # os.makedirs(replay_dir, exist_ok=True)
  train_replay = common.Replay(replay_dir, config.replay.capacity, config)
  print('Num episodes in train_replay: ', train_replay.stats)
  ...
  agnt = agent.Agent(config, obs_space, {'action': action_space}, step)
