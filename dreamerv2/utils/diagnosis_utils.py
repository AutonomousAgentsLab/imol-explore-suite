...
def dream(agnt, data, actions_for_imagine, save_dir,
    ...
    fn_img = f'{save_dir}_{expid.replace("/", "-")}_{name}_imgloss.{fig_suffix}'
    os.makedirs(save_dir, exist_ok=True)
    print(fn_gif)

    b = 1 # batchsize
    wm = agnt.wm
    if not 'is_terminal' in data.keys():
      data['is_terminal'] = np.zeros_like(data['discount'])  ### HACK to match new dv2 with original dv2
      data['is_first'] = np.zeros_like(data['discount'])  ### HACK to match new dv2 with original dv2
    ...
    prior = wm.rssm.imagine(data['action'][:b,
                            ...
                            init)
    #### SAVEOUT HERE? #####
    openl = wm.heads['decoder'](wm.rssm.get_feat(prior))['image'].mode()

    burnin_recon = wm.heads['decoder'](
      wm.rssm.get_feat(post))['image'].mode()[:b]
    ...
    if plot_image_loss:
      ...
      truth_imagine = data['image'][:b,
                      start_imagining_at:end_imagining_at]
      like_imagine = tf.cast(wm.heads['decoder'](wm.rssm.get_feat(prior))['image'].log_prob(truth_imagine), tf.float32)
      loss_imagine = -like_imagine.numpy()

      truth_burnin = data['image'][:b,
                     start_observing_at:start_imagining_at]
      like_burnin = tf.cast(wm.heads['decoder'](wm.rssm.get_feat(post))['image'].log_prob(truth_burnin), tf.float32)
