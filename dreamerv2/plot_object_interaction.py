...
def main():
  ...
  colors = {'object2': 'magenta',
            'object1': 'yellow'}  # Object name defined by admc_sphero_novel_object_unchanging environment.
  labels = {'object2': 'familiar',
            'object1': 'novel'}
  ball_ids = list(colors.keys())
  ball_ids = ['object2', 'object1']
  ...
  tends = tstart + np.round(np.array([200e3, 400e3, 600e3, 800e3, 1000e3])
                            / SAVERATE).astype(int)
  plot_indiv_trajectories(df, tstart, tends, ball_ids, colors,
                          titlestrs=['200k steps', '400k steps', '600k steps', '800k steps', '1000k steps'],
                          savepath=outdir,
                          )
