...
class Module(tf.Module):
  ...
  def save(self, filename):
    ...
    # Save out variables to make it easier to load checkpoint
    filename = str(filename)
    logdir = '/'.join(filename.split('/')[:-1])
    ckpt_name = filename.split('/')[-1]
    for_ckpt_dir = f'{logdir}/for_ckpt'
    os.makedirs(for_ckpt_dir, exist_ok=True)
    with open(f'{for_ckpt_dir}/config_{ckpt_name}', 'wb') as f:
      try:
        pickle.dump(self.config, f, protocol=pickle.HIGHEST_PROTOCOL)
      except:
        print(f'Not save self.config to {f}')
    with open(f'{for_ckpt_dir}/action_space_{ckpt_name}', 'wb') as f:
      try:
        pickle.dump(self.act_space, f, protocol=pickle.HIGHEST_PROTOCOL)
      except:
        print(f'Not save action_space_{ckpt_name}')
    with open(f'{for_ckpt_dir}/obs_space_{ckpt_name}', 'wb') as f:
      try:
        pickle.dump(self.obs_space, f, protocol=pickle.HIGHEST_PROTOCOL)
      except:
        print(f'Not save obs_space_{ckpt_name}')
