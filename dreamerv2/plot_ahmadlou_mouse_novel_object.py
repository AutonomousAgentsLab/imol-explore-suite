...
if __name__ == "__main__":
  ...
  plotpath = '/home/saal2/Dropbox/_gendreamer/plots/AHMADLOU/'
  ...
  for combine_all in [1]:
    ...
    plt.tight_layout()
    simple_axis(plt.gca())
    plt.savefig(f'{plotpath}/{mouse_type}_interactions.pdf')
    plt.show()
    ...
    plt.legend([p2, p1], ['Old', 'New'], loc='upper left', frameon=False,
             handlelength=0.5, handletextpad=0.5, bbox_to_anchor=[0.05, 1])
    plt.ylabel('Cumulative interactions')
    # plt.title(f'Interaction categories: {categories}')
    # plt.suptitle(f'{mouse_type}: Object {which_expt}, n={all_expt_new.shape[0]}')
    plt.suptitle(f'{mouse_type} Mouse, n={all_expt_new.shape[0]}')
    plt.xlabel('Time from new object introduction (s)')
    plt.ylim([0, 8000])
    simple_axis(plt.gca())
    plt.tight_layout()
    plt.savefig(f'{plotpath}/{mouse_type}_interactions_vs_time.pdf')
    plt.show()
